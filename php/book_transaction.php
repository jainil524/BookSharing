...
$SelectDeliveryItemQuery = "SELECT 
                                dg.delivery_guy_id AS dgID, 
                                (SELECT 
                                    COUNT(bt.delivery_guy_id) AS dd 
                                FROM 
                                    book_transaction bt 
                                WHERE 
                                    (bt.delivery_guy_id = dg.delivery_guy_id AND bt.delivery_guy_id IS NOT NULL) 
                                        AND 
                                    IsDelivered = 0 
                                GROUP BY 
